...
#'
#' For `cmd_background()`, `connection` cannot be used, and `TRUE` will fallback
#' to the empty string `""`, with a warning message.
#'
...
cmd_run <- function(command, stdout = TRUE, stderr = TRUE, stdin = NULL,
    ...
    stdout <- check_stdio(stdout, allow_null = FALSE)
...
#' @export
#' @rdname cmd_run
cmd_help <- function(command, stdout = TRUE, stderr = TRUE,
                     stdout_callback = NULL, stderr_callback = NULL,
                     verbose = TRUE) {
    assert_s3_class(command, "command")
    stdout <- check_stdio(stdout, allow_null = FALSE)
...
#' @export
#' @rdname cmd_run
cmd_background <- function(command, stdout = FALSE, stderr = FALSE,
                           stdin = NULL, verbose = TRUE) {
    assert_s3_class(command, "command")
    ...
    stdout <- check_stdio(stdout, allow_null = FALSE, allow_connection = FALSE)
    if (isTRUE(stdout)) {
...
check_stdio <- function(x, allow_null = TRUE,
                        allow_bool = TRUE,
                        allow_connection = TRUE,
                        arg = caller_arg(x), call = caller_call()) {
    if (rlang::is_string(x)) return(x) # styler: off
    if (rlang::is_bool(x)) {
        if (!allow_bool) {
            cli::cli_abort(
